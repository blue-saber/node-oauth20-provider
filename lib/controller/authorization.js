...
module.exports = function(req, res, next) {
    ...
    function(err) {
        if (err) response.error(req, res, err, redirectUri);
            ...
            if (req.method == 'GET')
                req.oauth2.decision(req, res, client, scope, user, redirectUri, authorization.code);
            else if (grantType == 'authorization_code')
                // authorization.code(req, res, client, scope, user, redirectUri);
                // Need a way to intercept before authorization.code been called.
                if (typeof (next) === 'function') {
                    // "next" will intercept before authorization.code been called.
                    // pass authorization.code to "next" as a callback function
                    next(req, res, client, scope, user, redirectUri, authorization.code);
                } else {
                    authorization.code(req, res, client, scope, user, redirectUri);
                }
